...
describe('ticketStore', () => {
  it('issues an opaque id that consumes exactly once', () => {
    ...
    const id = s.issue({ session: 'orca-advisor-1', userId: 1 });
    expect(id).toMatch(/^[a-f0-9]{32,}$/);
    expect(s.consume(id)).toEqual({ session: 'orca-advisor-1', userId: 1 });
  ...
  it('expires after ttl', () => {
    ...
    const id = s.issue({ session: 'orca-x', userId: null });
    t = 1500;

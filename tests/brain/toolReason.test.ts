...
describe('toolReason.withReason', () => {
  it('prepends an optional reason property to an object-schema tool, keeping the originals and required set', () => {
    const augmented = withReason(sampleTool('Write'));
    const p = props(augmented);
    expect(Object.keys(p)[0]).toBe('reason');            // reason first → the model authors it first
    expect(p.reason).toMatchObject({ description: REASON_DESC });
    expect(p.path).toBeDefined();
    expect(p.count).toBeDefined();
    // path stays required, reason/count optional.
  ...
  it('leaves excluded tools (ToolSearch, mcp__*) and non-object schemas untouched', () => {
    expect(props(withReason(sampleTool('ToolSearch'))).reason).toBeUndefined();
    expect(props(withReason(sampleTool('mcp__chrome__navigate'))).reason).toBeUndefined();
...
describe('toolReason.stripReason', () => {
  it('removes reason from the arguments before the real handler runs, without mutating the caller object', () => {
    const calls: unknown[] = [];
    const wrapped = stripReason(sampleTool('Write', calls));
    const args = { reason: 'Píšu soubor', path: 'a.ts', count: 2 };
    return wrapped.execute!('call-1', args as never, {} as never).then(() => {
      expect(calls).toHaveLength(1);
      expect(calls[0]).toEqual({ path: 'a.ts', count: 2 }); // reason gone
      expect(args).toHaveProperty('reason');                 // caller's object untouched (cloned, not mutated)
    });
  });

  it('passes arguments through untouched when no reason is present', () => {
    const calls: unknown[] = [];
    const wrapped = stripReason(sampleTool('Write', calls));
...
describe('toolReason.extractReason', () => {
  it('returns a non-empty string reason, else undefined', () => {
    expect(extractReason({ reason: 'Čtu konfiguraci', path: 'x' })).toBe('Čtu konfiguraci');
    expect(extractReason({ reason: '   ' })).toBeUndefined();
    expect(extractReason({ path: 'x' })).toBeUndefined();
    expect(extractReason({ reason: 42 })).toBeUndefined();

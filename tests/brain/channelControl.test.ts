...
describe('ChannelSessionService — channel-scoped slash control (stop/status/compact)', () => {
  ...
  it('compact runs session.compact under the lock and returns post-compaction usage; null if no session', async () => {
    const ch = fakeChannel();
    const svc = serviceWith(new Map([['discord-c1#0', ch]]));
    const usage = await svc.compact('discord-c1#0');
    expect((ch.session.compact as ReturnType<typeof vi.fn>)).toHaveBeenCalledOnce();
    expect(usage).toMatchObject({ tokens: 1200, contextWindow: 8000 });

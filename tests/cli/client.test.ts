...
describe('ElowenClient', () => {
  it('createTask POSTs to /tasks', async () => {
    const calls: any[] = [];
    global.fetch = vi.fn(async (url: any, init: any) => { calls.push({ url, init }); return new Response(JSON.stringify({ id: 'elowen-1' }), { status: 201 }); }) as any;
  ...
  it('close PATCHes the task to closed and sends the bearer token', async () => {
    const calls: any[] = [];
    global.fetch = vi.fn(async (url: any, init: any) => { calls.push({ url, init }); return new Response(JSON.stringify({ id: 'elowen-1', status: 'closed' }), { status: 200 }); }) as any;
    const c = new ElowenClient('http://localhost:4400', 'svc-token');

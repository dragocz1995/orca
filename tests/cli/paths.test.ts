...
describe('cli/paths', () => {
  ...
  it('defaults the data dir to ~/.config/orca', () => {
    expect(dataDir(env)).toBe('/h/.config/orca');
  });
  it('derives the db path under the data dir', () => {
    expect(dbPath(env)).toBe('/h/.config/orca/orca.db');
  });

...
describe('chat application shell ownership', () => {
  ...
  it('leaves Ctrl+B to the native editor when nothing foreground can be detached', () => {
  ...
  it('claims Ctrl+B for the dispatch when a running foreground command is present', () => {
    let listener!: (data: string) => { consume: boolean } | undefined;
    const tui = { addInputListener: vi.fn((next) => { listener = next; return vi.fn(); }) } as unknown as TUI;
    const dispatchAction = vi.fn();
    const context = {
      state: { childView: null, processes: [{ id: 'p1', running: true, completionMode: 'foreground' }] },
      term: { columns: 80, write: vi.fn() },
      editor: { focused: true, getText: () => 'abc' },
      stream: { subagentStates: () => [], workflowStates: () => [] }, quit: vi.fn(), renderForced: vi.fn(),
      keymap: () => ({ matches: () => false, isLeader: () => false, directAction: () => 'subagent_background' }),

...
describe('cli-settings routes', () => {
  it('GET returns defaults + the server default model', async () => {
    ...
    expect(await res.json()).toEqual({ model: '', modelProvider: '', visionModel: '', visionModelProvider: '', autoCompact: false, autoCompactAt: 80, advisorStyle: 'professional', discordUserId: '', serverDefault: 'claude-opus-4-8' });
  });
  ...
  it('PATCH saves the override and restarts a running brain', async () => {
    ...
    expect(await res.json()).toEqual({ model: 'ollama/kimi-k2.7-code', modelProvider: 'relay', visionModel: '', visionModelProvider: '', autoCompact: true, autoCompactAt: 70, advisorStyle: 'professional', discordUserId: '', serverDefault: 'claude-opus-4-8' });

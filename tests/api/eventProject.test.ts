...
// A fixed lookup table standing in for the real stores: t1→1, epicA→2, session orca-Nova→3, job1→4.
const deps: EventProjectDeps = {
  taskProject: (id) => (({ t1: 1, epicA: 2 }) as Record<string, number>)[id] ?? null,
  sessionProject: (s) => (s === 'orca-Nova' ? 3 : null),
  jobProject: (id) => (id === 'job1' ? 4 : null),
};
const signal = (session: string): OrcaEvent => ({ type: 'signal', session, signal: { type: 'needs_input' } as never });
...
describe('eventProjectId — single source of project resolution', () => {
  ...
  it('resolves a signal via the session it runs in', () => {
    expect(eventProjectId(signal('orca-Nova'), deps)).toBe(3);
  });
  ...
  it('fails closed (null) when the referenced row is gone', () => {
    ...
    expect(eventProjectId(signal('orca-Ghost'), deps)).toBeNull();
  });
});

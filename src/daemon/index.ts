...
import { migrateLegacyConfigDir } from '../cli/paths.js';

const log = logger('daemon');

// Bridge the legacy ~/.config/orca state dir onto the new ~/.config/elowen default before the DB opens,
// so an in-place upgrade past the rebrand keeps its existing database instead of starting empty.
...
// Runtime env, ELOWEN_* primary with an ORCA_* backward-compat fallback (existing prod systemd + user
// shells still set the old names). Bound to locals so control-flow narrowing works in the guards below.
const relayUrl = process.env.ELOWEN_RELAY_URL ?? process.env.ORCA_RELAY_URL;
const bootstrapUser = process.env.ELOWEN_BOOTSTRAP_USER ?? process.env.ORCA_BOOTSTRAP_USER;
const bootstrapPass = process.env.ELOWEN_BOOTSTRAP_PASS ?? process.env.ORCA_BOOTSTRAP_PASS;

const { app, startLoops, tickets, tmux } = buildApp({
  dbPath: (process.env.ELOWEN_DB ?? process.env.ORCA_DB) ?? `${process.env.HOME}/.config/elowen/elowen.db`,
  project: { id: 1, slug: (process.env.ELOWEN_PROJECT ?? process.env.ORCA_PROJECT) ?? 'elowen', path: (process.env.ELOWEN_PROJECT_PATH ?? process.env.ORCA_PROJECT_PATH) ?? process.cwd() },
  relay: relayUrl ? { baseUrl: relayUrl, apiKey: (process.env.ELOWEN_RELAY_KEY ?? process.env.ORCA_RELAY_KEY) ?? '', model: (process.env.ELOWEN_RELAY_MODEL ?? process.env.ORCA_RELAY_MODEL) ?? 'gpt-4o-mini' } : null,
  bootstrap: bootstrapUser && bootstrapPass ? { username: bootstrapUser, password: bootstrapPass } : null,
  allowOpen: (process.env.ELOWEN_ALLOW_OPEN ?? process.env.ORCA_ALLOW_OPEN) === '1',
});
...
const host = (process.env.ELOWEN_HOST ?? process.env.ORCA_HOST) ?? '127.0.0.1';
const server = serve({ fetch: app.fetch, port: Number((process.env.ELOWEN_PORT ?? process.env.ORCA_PORT) ?? 4400), hostname: host }, info => log.info(`elowen serve on ${host}:${info.port} — logs → ${LOG_DIR}`));
...
server.on('error', (e: NodeJS.ErrnoException) => {
  if (e.code === 'EADDRINUSE') log.error(`port ${(process.env.ELOWEN_PORT ?? process.env.ORCA_PORT) ?? 4400} already in use, exiting`);

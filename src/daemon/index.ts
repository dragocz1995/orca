...
const { app, startLoops } = buildApp({
  ...
  relay: process.env.ORCA_RELAY_URL ? { baseUrl: process.env.ORCA_RELAY_URL, apiKey: process.env.ORCA_RELAY_KEY ?? '', model: process.env.ORCA_RELAY_MODEL ?? 'mimo-v2.5' } : null,

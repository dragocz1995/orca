...
export function createServer(d: ServerDeps): Hono<{ Variables: { user: User; token: string; tokenScope: TokenScope } }> {
  // Shared per-server state + access/path helpers, built once and threaded into every route family.
  // Destructured here so the route handlers still in this file read the same names they always did.
  const ctx = createRouteContext(d);
  const {
    log, planJobs, decisionQueue, tickets, gitLock,
    canAccessProject, notAdmin, accessibleProjects, missionAccessible,
    eventDeps, sessionAccessible, execAllowedForUser,

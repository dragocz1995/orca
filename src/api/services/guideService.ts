...
export interface GuideService {
  /** Render the context-aware Elowen control guide for a task (what `elowen help` prints to a running
...
/** Backs `elowen help` (GET /tasks/:id/guide). The agent's spawn preamble is deliberately short and just
...
export function createGuideService(d: ServerDeps): GuideService {
  ...
  function render(taskId: string): string | null {
    ...
    // worker preamble tells a phase agent to run `elowen help` BEFORE it starts (while the mission is still

/** Fetch elowen's latest published version from the npm registry, or null on ANY failure (network
...
export async function fetchLatestVersion(fetchFn: typeof fetch = fetch): Promise<string | null> {
  try {
    const r = await fetchFn('https://registry.npmjs.org/elowen/latest');
    if (!r.ok) return null;

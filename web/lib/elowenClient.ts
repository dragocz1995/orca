...
export const elowenClient = {
  ...
  savePluginSubagent: (name: string, def: { description: string; tools: 'read-only' | 'all' | 'inherit' | string[]; body: string }) =>
    req<{ ok: boolean }>(`/plugins/agents/${encodeURIComponent(name)}`, json(def, 'PUT')),
  deletePluginSubagent: (name: string) => req<{ ok: boolean }>(`/plugins/agents/${encodeURIComponent(name)}`, { method: 'DELETE' }),

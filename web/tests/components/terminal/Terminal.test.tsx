...
vi.mock('@xterm/xterm', () => ({
  Terminal: class { open = openSpy; write = writeSpy; clear = clearSpy; dispose = disposeSpy; loadAddon = vi.fn(); onData = (fn: (d: string) => void) => { onDataHandler = fn; return { dispose: vi.fn() }; }; },
}));
...
describe('Terminal', () => {
  ...
  it('interactive terminal forwards every keystroke verbatim to the input endpoint', () => {
    render(<Terminal name="orca-E" interactive />);

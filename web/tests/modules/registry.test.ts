...
describe('module registry', () => {
  it('registers the nine modules with routes + groups', () => {
    expect(MODULES.map((m) => m.route)).toEqual(['/dash', '/tasks', '/kanban', '/timeline', '/escalations', '/sessions', '/settings', '/projects', '/users']);
    expect(MODULES.every((m) => typeof m.icon !== 'undefined')).toBe(true);
  });
  it('groups Operate (7) and Config (2)', () => {
    const groups = modulesByGroup();
    expect(groups.find((g) => g.group === 'Operate')?.items.length).toBe(7); // missions folded into tasks; escalations + projects in Operate

// Streaming/edit-throttle machinery: the live progress bubble and the final-answer posting. Ported from
// the Discord adapter's stream.mjs — the rendering logic (foldedCalls / toolLinesFor / cardLines /
// StreamingAnswer / per-tool bubbles / the brain-event protocol) is preserved; only the transport is
// swapped to grammY (bot.api.sendMessage / editMessageText / deleteMessage), and the Discord-markdown
// decorations are rendered as plain text (Telegram messages are sent without a parse_mode).
...
const EDIT_THROTTLE_MS = 1200; // Telegram edit rate — stay well under it (matches the Discord throttle)
...
/** One editable Telegram message: created on the first write, then edited in place (throttled). Shared by
 *  the tool-progress bubble and the streaming answer. Callers keep content within Telegram's limit
 *  (progress slices to CHUNK; the answer pre-splits with splitContent). */
class EditableMessage {
  constructor(adapter, chatId, createExtra = {}) {
    this.a = adapter;
    this.chatId = chatId;
    this.createExtra = createExtra; // merged into the FIRST send only (e.g. a reply anchor); edits stay plain
    this.messageId = null;
    this.content = '';
    this.sent = null;    // the content Telegram last actually received — skip a redundant edit when unchanged
    this.lastEdit = 0;
   ...
   *  when the content Telegram already has is current, so a coalesced/duplicate flush is a no-op. */
  async drain() {
    if (this.closed) return;
    if (this.content === this.sent) return; // Telegram already has the latest content
    this.lastEdit = Date.now();
    const content = this.content;
    if (!this.messageId) {
      const id = await this.a.tgSend(this.chatId, content || '💭 …', this.createExtra);
      this.messageId = id ?? null;
      if (this.messageId) this.sent = content; // a failed send leaves sent null so the next drain retries the create
    } else {
      ...
      const ok = await this.a.tgEdit(this.chatId, this.messageId, content);
      if (ok) this.sent = content;
    }
  }
}

/** The streaming answer: the assistant's reply text edited live into its OWN Telegram message(s), kept
 *  separate from the tool-progress bubble so alternating text→tool→text never loses an edit target.
 *  Overflow past one message is split code-fence-aware (splitContent) into ordered continuation bubbles;
 *  only the last (growing) one is edited on each delta — earlier ones freeze at their piece. The FIRST
 *  bubble is a real reply to the triggering message. */
class StreamingAnswer {
  constructor(adapter, chatId, replyToId) {
    this.a = adapter;
    this.chatId = chatId;
    this.replyToId = replyToId; // the first bubble replies to it; continuations are plain
    this.bubbles = [];          // ordered EditableMessages; index 0 carries the reply anchor
  }
  bubbleFor(i) {
    let b = this.bubbles[i];
    if (!b) {
      const ref = i === 0 && this.replyToId ? { reply_parameters: { message_id: this.replyToId, allow_sending_without_reply: true } } : {};
      b = new EditableMessage(this.a, this.chatId, ref);
      // Serialize the initial create ACROSS bubbles through one linked chain: a new bubble's first send
      // waits for the previous bubble's send chain, so continuation posts land in piece order.
  ...
  /** Freeze a bubble and delete its Telegram message, AWAITING any in-flight create/edit first — a bubble
   *  whose initial send is still in flight has messageId === null, so deciding close-vs-delete before the
   *  send settles would let a straggler create escape and leave an orphan message behind. */
  async deleteBubble(b) {
    b.close();
    await b.sending?.catch(() => {}); // let the in-flight create/edit settle so messageId is known
    if (b.messageId) await this.a.tgDelete(b.chatId, b.messageId);
  }
...
// The live-trace render/fold engine is shared (../../_shared/liveTrace.mjs). Telegram sends plain text
// (no parse_mode), so its style is all-identity — nothing to escape (mentions/fences are inert) and no
// bold/strike markers — with a plain `  ↳` output line. The fold rule and summaries come from the core.
const style = {
  mentionSafe: (s) => s,
  fenceSafe: (s) => s,
  bold: (s) => s,
 ...
 *  surfaces independent lets Telegram feel alive without forcing token-by-token answer noise. */
export class LiveMessage {
  constructor(adapter, chatId, replyToId, askerId, display) {
    this.a = adapter;
    ...
    this.toolBubbles = new Map(); // per-tool editable bubbles when the chat opts into per_tool layout
   ...
   *  apart by a thin divider — so the plan never reads as just another tool line. */
  renderProgress() {
    const toolLines = [];
    // The step counter is a STALL hint, not always-on: it surfaces only once the turn has gone
    // STALL_HINT_MS with no new tool/card, so a long step doesn't read as a frozen agent.
    ...
    // In per-tool mode each lifecycle row owns a separate editable message; the aggregate progress bubble
    // is reserved for cards/notices/reasoning so those surfaces remain coherent.
    ...
    // Preserve the newest/active tools and cards when a long turn exceeds Telegram's message limit.
  ...
  onEvent(e) {
    if (e.type === 'tool' && e.name) {
      ...
      // Off by default — reasoning is noise. When cfg.showReasoning is on it rides the progress bubble as
      // a dim tail. Always accumulated so toggling mid-turn has content to show.
      ...
      // turn stays blocked in the tool until the user answers via a button/text interaction).
  ...
  async finalize(reply) {
    // Settle the progress bubble to its complete tool list (a throttled edit may still be pending), then
    // freeze it so the straggler timer can't fire afterwards.
    clearTimeout(this.stallTimer);
    ...
    // BELOW the trace — restoring the guarantee that the final answer is the chat's LAST message.
    if (this.answerStranded && this.answer) { await this.answer.discard(); this.answer = null; }
    // strip any leaked <think> reasoning (vision-fallback models) before it ever reaches the chat.
    ...
    // is posted BEFORE the text so it reads as a standalone attachment above the footer.
    const { cleaned, files } = extractImageRefs(full);
    const names = new Set(files);
    for (const ref of this.imageRefs) names.add(ref.slice(ref.lastIndexOf('/') + 1));
    let posted = false;
    if (names.size && typeof this.a.resolveImageFiles === 'function' && typeof this.a.sendPhotos === 'function') {
      const data = this.a.resolveImageFiles([...names]);
      if (data.length) { await this.a.sendPhotos(this.channelId, data, {}).catch(() => {}); posted = true; }
    }
